...
const getRefreshToken=async (req)=>{
    ...
    const comparasons=await Promise.all(userTokens.map(t=>t.compareToken(refreshToken)));
    const matchedIndex=comparasons.findIndex(m=>m===true);
    if(matchedIndex === -1)
...
export const refresh=async (req,res,next)=>{
    try{
        const userTaken=await getRefreshToken(req);
        const accessToken=jwt.sign({userId:userTaken.user},
            process.env.JWT_ACCESS_SECRET,
            {expiresIn:process.env.JWT_ACCESS_EXPIRES_IN}
        );
